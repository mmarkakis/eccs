...
def main():
    ...
    starting_graph = None
    if config["gen_starting_graph"]:
        method = config["gen_starting_graph"]["method"]
        option = config["gen_starting_graph"]["option"]
        starting_graph, _ = CausalDiscovery.run_with_timer(method, option)
        nx.nx_pydot.write_dot(
            starting_graph,
            os.path.join(args.out_path, f"{dataset_name}_{method}_{option}_graph.dot"),
        )
    elif config["load_starting_graph"]:
        ...
        starting_graph = nx.DiGraph(
            nx.nx_pydot.read_dot(
                os.path.join(path, f"{dataset_name}_{method}_{option}_graph.dot")
            )
        )
    else:
    ...
    if config["run_eccs"]:
        ...
        np.save(
            os.path.join(
                args.out_path, f"{dataset_name}_{method}_{num_steps}_ate_trajectory.npy"
            ),
            ate_trajectory,
        )
        np.save(
            os.path.join(
                args.out_path,
                f"{dataset_name}_{method}_{num_steps}_edit_distance_trajectory.npy",
            ),
            edit_distance_trajectory,
        )
    elif config["load_eccs"]:
        path = config["load_eccs"]["path"]
        method = config["load_eccs"]["method"]
        ...
        raise ValueError("No ECCS running or loading parameters specified")
